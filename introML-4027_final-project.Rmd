...
Looks like we should be using 4 randomly selected variables for each predictor, as it maximizes the accuracy rate of prediction.
...
Below is an ROC Curve representing the accuracy of prediction for whether or not an event was a "signal" event.  The predicted possibilities of an event are used, which is why the curve appears continuous. 
...
Typically, random chance is represented by an AUC of 0.5.  We can see here that the AUC of predicting a signal event is roughly 0.89, so we're definitely picking up the signal.  However, this doesn't tell us exactly what the accuracy rate is, just that we're discriminating between background and signal events much better than random chance.  The following will address this. 
...
We observe that the optimum threshold to balance specificity and sensitivity is at coordinate 0.3665 on the curve, according to the ROC curve we generated.  
The accuracy and precision at this optimum threshold is 0.814 and 0.69.  
The true positive rate and true negative rate are roughly the same _at this optimum threshold_, approximately 0.819 and 0.812 respectively. 

However, these are measures of being able to discriminate between a background event and a signal event.  This hasn't told us how well the model actually performed on the validation set, just that based on the validation set, it will probably generalize well to a larger population data set. 
...
Something important to note here is that the "positive" class is a background event.  
If we're looking to determine how well the model is classifying signal events, we need to look at the "negative" class, for the above confusion matrix. 
...
The "No Information Rate", or, the accuracy of random guesses based on the distribution, is 0.6597.  
In order to be better than random guessing, the different prediction accuracies must be greater than this value.  
At first glance, this is definitely the case, the overall accuracy rate is 0.8295!  
...
While our overall accuracy rate is 82.95%, our accuracy rate for the classification of signal events (Neg Pred Value, above) is 77.63%.  
This is definitely higher than the no information rate of 0.6597.  However, it's rather low, only performing 11.66% better than the No Information Rate.  

The accuracy rate for determining if something is a background event is 85.31%.  
This is definitely high relative to the No Information Rate. 


### Interpreting Accuracy and ROC

Overall, the model is discriminating well between the background and signal events.  The evidence for this was in the ROC curve, and the associated AUC metric.  

However, the accuracy rate says that we are only predicting background events well, and we are not predicting signal events as well as the background events.  However, the accuracy rate of true identification for both is well above the No Information Rate.  

Overall, it appears the model does _okay_, it's not overwhelmingly good at prediction with a validation accuracy rate of 0.8295.  However, the metrics we have available to evaluate the model indicate that it performs relatively well enough for determining variable importance. 
...
Where methods such as regression and other techniques are accomplishable with a real solid beer or two, Random Decision Forests ended up being much more intense - the author's suggestion is to acquire boxed wine, friends, and cheese and crackers of choice.  The author found most the best success with the leverage of toasted brie and goat cheese, with a multiple berry preserve or jam, applied to a Ritz cracker. 
